...
// drawio's vendored viewer.min.js contains createElement("script") call sites
// that trip the plugin-review scanner. Offline previews never need them:
//   1. MathJax-from-CDN loader (createElement + src=URL). Dead offline.
//   2. addSvgIconHandlers HTML fallback (createElementNS(NS_SVG,"script")
//      else createElement("script")). Preview uses GraphViewer, not EditorUi
//      SVG export; we only rewrite the HTML fallback so the scanner string
//      is gone. Each snippet's match count is asserted so a drawio bump that
//      changes the minified shape fails the build loudly.
...
const VIEWER_SVG_SCRIPT_FALLBACK = 'f.createElement("script")';
...
const sanitizeDrawioViewerPlugin = {
  ...
  setup(build) {
    build.onLoad({ filter: /viewer\.min\.txt$/ }, async (args) => {
      ...
      const replacements = [
        { label: 'VIEWER_SCRIPT_LOADER', from: VIEWER_SCRIPT_LOADER, to: '' },
        { label: 'VIEWER_SVG_SCRIPT_FALLBACK', from: VIEWER_SVG_SCRIPT_FALLBACK, to: 'null' },
      ];
      ...
      for (const { label, from, to } of replacements) {
        ...
        if (count !== 1) {
          throw new Error(
            `sanitize-drawio-viewer: expected exactly 1 ${label} in ` +
            `viewer.min.txt, found ${count}. The vendored drawio viewer likely ` +
            `changed; review and update the snippet in esbuild.config.mjs.`,
          );
        }
      ...
      if (contents.includes('createElement("script")') || contents.includes("createElement('script')")) {
        throw new Error(
          ...
          'stripping the known snippets; aborting to avoid shipping it.',
        );
      }
